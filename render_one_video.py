...
def render_one_video(args, cam_num, save_video=0, save_npy=0):
    ...
    c2w = poses[cam_num, :, :] # 0, 1, 2 카메라 번호 명시, 0번째 카메라 위치에서 렌더링 진행, 
    ...
    with torch.no_grad():
        ...
        if save_npy:
            depth_save_path_ = depth_savePath + '/depth' + '/depthmap_cam{:02}.npy'.format(cam_num)
...
if __name__=='__main__':
    args = config_parser()
    render_one_video(args, args.cam_num, 1, 0)